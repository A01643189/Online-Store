...
import { createOrder, updateOrderPaymentStatus } from '../services/orderServices.ts';
...
export const Checkout = () => {
  ...
  const [cartItems, setCartItems] = useState<Array<{ product: Product; cartItem: CartItem }> | null>(null);
  ...
  useEffect(() => {
    if (cart && products) {
      ...
      }).filter(Boolean) as Array<{ product: Product; cartItem: CartItem }>;
      setCartItems(items);
    }
  }, [cart, products]);
  ...
  useEffect(() => {
    if (address.postalCode && cartItems) {
      ...
      calculateShipping({ postalCode: address.postalCode, weight: totalWeight }).then((rates) => {
        setShippingOptions(rates);
        if (rates.length > 0) {
          const cheapest = rates.reduce((prev, current) => prev.amount < current.amount ? prev : current);
  ...
  useEffect(() => {
    const isValid = address.fullName.trim() !== '' && address.street.trim() !== '' && isShippingSelected;
  ...
  const calculateSubtotal = () => {
    ...
    return cartItems.reduce((total, { product, cartItem }) => total + (product.price * cartItem.quantity), 0);
  };
  ...
  const onApproveOrder = async (data: any, actions: any) => {
    return actions.order.capture().then(async (details: any) => {
      ...
      if (!cartItems || !selectedShipping) throw new Error("Cart items or shipping method not selected");
  ...
  console.log('Valid:', isAddressValid, selectedShipping, address);
      ...
      <form className="checkout-container">
        <div className="shipping-form">
          ...
          <div className="form-group">
            ...
            <PostalCodeLookup
              onSelect={(data) => setAddress(prev => ({ ...prev, ...data, postalCode: data.postalCode || '' }))}
                    ...
                    <label htmlFor={`shipping-${index}`}>
                      <div className="shipping-option-content">
                        ...
                        <div className="shipping-details">
                          <div className="shipping-service"><span>{option.service}</span></div>
                          <span className="shipping-price">${option.amount.toFixed(2)}</span>
        ...
        <div className="order-summary">
          ...
          <div className="order-items">
            {cartItems.map(({ product, cartItem }) => (
              <CartItemCard key={product.id} product={product} cartItem={cartItem} />
            ))}
          </div>
          ...
          {isAddressValid ? (
            ...
            <div className="payment-requirements-message">
              {!address.fullName.trim() && <div className="requirement">• Please enter your full name</div>}
              {!address.street.trim() && <div className="requirement">• Please enter your street address</div>}
              {!isShippingSelected && <div className="requirement">• Please select a shipping option</div>}
            </div>
          )}
        </div>
      </form>
